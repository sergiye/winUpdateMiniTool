...
public class ListViewExtended : ListView {
  private const int MfLvmFirst = 0x1000; // ListView messages
  private const int MfLvmSetgroupinfo = MfLvmFirst + 147; // ListView messages Setinfo on Group
  private const int MfWmLbuttonup = 0x0202; // Windows message left button
    ...
    if (lstvwgrp.ListView.InvokeRequired) {
      ...
      try {
        ...
        if (grpId != null) {
          ...
          SendMessage(lstvwgrp.ListView.Handle, MfLvmSetgroupinfo, (IntPtr)grpId.Value, ip);
          SendMessage(lstvwgrp.ListView.Handle, MfLvmSetgroupinfo, (IntPtr)grpId.Value, ip);
        }
        else {
          group.IGroupId = gIndex;
          Marshal.StructureToPtr(group, ip, false);
          SendMessage(lstvwgrp.ListView.Handle, MfLvmSetgroupinfo, (IntPtr)gIndex, ip);
          SendMessage(lstvwgrp.ListView.Handle, MfLvmSetgroupinfo, (IntPtr)gIndex, ip);
        }
